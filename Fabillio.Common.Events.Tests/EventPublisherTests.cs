...
public class EventPublisherTests: RavenDbTests
{
    ...
    public void Setup()
    {
        ...
        A.CallTo(() => _daprClient.PublishEventAsync(A<string>.Ignored, "", null, CancellationToken.None))
    ...
    public async Task Publish_Always_PublishesDaprEvent()
    {
        ...
        // Assert
        A.CallTo( () => _daprClient.PublishEventAsync(
    ...
    public async Task PublishWithOutbox_WhenDaprFails_SavesOutboxEvent()
    {
        ...
        var target = new EventPublisher(_daprClient, documentStore, _logger);
        A.CallTo(() => _daprClient.PublishEventAsync(EventPublisher.PubSub,
            "test-event-without-attribute",
            testEvent,
            CancellationToken.None)).Throws(() => new Exception("Some exception"));
        ...
        var outboxEvents = await documentSession.Query<OutboxEvent>()
            .ToListAsync();
