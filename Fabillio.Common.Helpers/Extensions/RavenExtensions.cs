...
{
    public static IRavenQueryable<TSource> WithPagination<TSource>(this IRavenQueryable<TSource> source, int page,
        int size, Action<SortingParams> sortingParams = null)
    {
    ...
    public static IRavenQueryable<TSource> OrderByField<TSource>(this IRavenQueryable<TSource> source, string sortField, bool isDescending)
    {
        var type = typeof(TSource);

        var propertyInfo = type.GetProperty(sortField, BindingFlags.IgnoreCase | BindingFlags.Public | BindingFlags.Instance);

        if (propertyInfo == null)
        {
            throw new ArgumentException($"Could not find a property named '{sortField}' on type '{type.FullName}'.");
        }
        ...
        var resultExp = Expression.Call(typeof(Queryable), method, types, source.Expression, Expression.Quote(orderByExp));
