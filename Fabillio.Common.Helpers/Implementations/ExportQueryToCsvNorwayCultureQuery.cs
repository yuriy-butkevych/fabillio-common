...
public static class ExportQueryToCsvNorwayCultureQuery
{
    ...
    public static byte[] CreateCSVEncodedResults<TToExport, TMapper>(List<TToExport> dataToExport,
        Action<CustomConverterOptions> action) where TMapper : ClassMap
    {
        ...
        {
            Delimiter = _csvTextDelimiter, IgnoreBlankLines = false
        };
        ...
        using (var csvWriter = new CsvWriter(streamWriter, configurations))
        {
            if (options.Decimal)
            {
                csvWriter.Context.TypeConverterCache.AddConverter<decimal>(new CustomDecimalConverter());
            }

            if (options.DateTime)
            {
                csvWriter.Context.TypeConverterCache.AddConverter<DateTime>(new CustomDateTimeConverter());
            }
    ...
    public static async Task<List<TExport>> MapFromFormFile<TExport>(IFormFile formFile)
    {
        ...
        using (var streamReader = new StreamReader(formFile.OpenReadStream()))
        {
            using var csvReader = new CsvReader(streamReader,
                ...
                CultureHelper.GetCustomCultureInfo())
                {
